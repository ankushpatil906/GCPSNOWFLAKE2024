...
class SnowflakeConnector:  # Fixed class name typo
    def __init__(self, account, user, password, warehouse, database, schema, role):
        self.account = account
        self.user = user.strip()  # Stripped extra whitespace
    ...
    def connect(self):
        self.connection = snowflake.connector.connect(
            ...
            schema=self.schema,
            role=self.role  # Added missing role parameter
        )
        self.cursor = self.connection.cursor()

    def execute_query(self, query):  # Fixed method name typo
...
user = "ANKUSHPATIL"  # Removed leading space
